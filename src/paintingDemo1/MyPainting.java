...
public class MyPainting extends JFrame // 主类，扩展了JFrame类，用来生成主界面
{
	...
	private String names[] = { "新建", "打开", "保存", // 这三个是基本操作按钮，包括"新建"、"打开"、"保存"
	...
	private String tipText[] = {
			...
			// 读者可以参照上面的按钮定义对照着理解
			"新建一幅新图", "打开已保存图像", "保存现有图像", "鼠标拖动随意画", "绘制一条直线", "绘制空心矩形", "绘制以指定颜色填充的实心椭圆", "绘制空心椭圆", "绘制以指定颜色填充的实心椭圆",
			"绘制圆形", "绘制以指定颜色填充的实心圆形", "绘制空心圆角矩形", "绘制以指定颜色填充的实心圆角矩形", "橡皮擦，可用来擦去已经绘制好的图案", "选择颜色按钮，可用来选择需要的颜色",
			"选择线条粗细的按钮", "输入文字按钮" };
	...
	public void chooseColor() {
		color = JColorChooser.showDialog(MyPainting.this, "Choose a color", color);
		R = color.getRed();
		G = color.getGreen();
		B = color.getBlue();
		itemList[index].R = R;
	...
	public void setStroke() {
		String input;
		input = JOptionPane.showInputDialog("请输入一个大于0的数");
		try { // 异常处理判断————必须输入数字
	...
	public void loadFile() {
		...
		fileName.canRead();
		if (fileName == null || fileName.getName().equals(""))
			JOptionPane.showMessageDialog(fileChooser, "不规范的名字", "不规范的名字", JOptionPane.ERROR_MESSAGE);
		else {
			try {
